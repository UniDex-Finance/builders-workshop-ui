...
import { BalanceSection } from './sidebar/balance-section'
import { PositionsSection } from './sidebar/positions-section'
...
export function EmiliaChat() {
  ...
  const { chatHistory, isLoading, sendMessage } = useCallApi({ 
    balance: tradingBalance  // Use the actual balance instead of hardcoded value
  })
  ...
  return (
    <div className="flex flex-col h-screen bg-background text-foreground">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar */}
        <div className="hidden border-r md:flex md:w-80 md:flex-col border-zinc-800 bg-background">
          {/* Fixed Balance Section */}
          <div className="flex-none">
            <BalanceSection onBalanceChange={handleBalanceChange} />
          </div>
          {/* Scrollable Sections */}
            ...
            <div className="p-4">
              <div className="max-w-3xl mx-auto">
                <div className="relative">
                  <textarea
                    className="w-full bg-zinc-900 text-white placeholder-zinc-400 rounded-lg p-4 pr-16 pb-12 resize-none border border-zinc-800 focus:outline-none focus:ring-1 focus:ring-zinc-700 min-h-[60px]"
                    placeholder={chatHistory.length === 0 ? "Ask me about a trade I can place" : "Ask a follow up..."}
                    value={message}
                  ...
                  <Button 
                    size="icon"
                    className="absolute -translate-y-[75%] right-3 top-1/2 bg-zinc-800 hover:bg-zinc-700"
                  ...
                  {/* Info Section inside textarea */}
                  <div className="absolute flex items-center justify-between bottom-3 left-3 right-3">
                    <Badge 
                      variant="outline" 
                      className="bg-zinc-900/50 text-zinc-400 hover:bg-zinc-900/50 rounded-none px-2 py-0.5 text-xs border-zinc-700"
                    >
                      Referrer: 0xKrunal
                    </Badge>
