...
export function ActionsCard({ isStaking, setIsStaking }: ActionsCardProps) {
  ...
  const needsApproval = () => {
    ...
    try {
      const parsedAmount =
        action === "mint" ? parseUnits(amount, 6) : parseUnits(amount, 18); // Use 18 decimals for USD.m approvals
      return action === "mint"
  ...
  const handlePercentageClick = (percentage: number) => {
    if (action === "mint") {
      ...
      const value = (
        Number(usdmData?.formattedUsdmBalance || 0) * percentage
      ).toFixed(6);
      setAmount(value);
    }
  };
  ...
  const calculateUsdValue = (inputAmount: string) => {
    ...
    const price =
      action === "mint" ? usdcPrice : Number(usdmData?.formattedUsdmPrice || 0);
    ...
    <Card className="bg-[#16161D] border-[#1b1b22]">
      ...
      <CardContent className="pt-4 space-y-6">
        <div className="space-y-4">
          ...
          <div className="text-sm text-[#A0AEC0]">{getAvailableBalance()}</div>
              ...
              <div className="flex flex-col gap-2">
                <div className="flex justify-between text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      {amount ? calculateOutputAmount(amount) : "0.00"}
                    </span>
                    <span className="text-[#A0AEC0]">
                      {action === "mint" ? "USD.m" : "USDC"}
                    </span>
                  </div>
                </div>
                <div className="flex justify-between text-sm">
